...
export default function ConfFormSubCategorias() {
  ...
  const HandledEdit = async () => {
    if (subCategoriaSelected.subCategoriaID == 0)
      return alert("Debe seleccionar Subcategoría a Modificar");
    try {
      setFormState("edicion");
      const categorias = await getAllCategorias();
      setListadoCategorias(categorias);
  ...
  const HandledEliminar = async () => {
    if (subCategoriaSelected.subCategoriaID == 0)
      return alert("Debe seleccionar Subcategoría a Eliminar");
    try {
      const ok = window.confirm("¿Eliminar categoría?");
      if (!ok) return;
      const eliminar = await deleteSubcategorias(
  ...
  const handledCancel = () => {
    if (subCategoriaSelected.subCategoriaID === 0) return;
    setSubCategoriaSelected(emptySubCategorias);
    setFormState("lectura");
  };
  return (
          ...
          >
            ...
            <tbody>
              {listadoSubcategorias.map((d, i) => (
                <tr
                  key={i}
                  onDoubleClick={() => setSubCategoriaSelected(d)}
                  className={
                    subCategoriaSelected?.subCategoriaID == d.subCategoriaID
                      ? s.selectedRow
                      : ""
                  }
                >
                  ...
                  <td>{d.estado === 1 ? "Inactivo" : "Activo"}</td>
                </tr>
              ))}
            </tbody>
          </table>
